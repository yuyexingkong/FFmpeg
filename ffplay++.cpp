...
{
    vfilters_list = (const char **) grow_array((void*)vfilters_list , sizeof(*vfilters_list ), &nb_vfilters,nb_vfilters + 1);
...
static void video_refresh(void *opaque, double *remaining_time)
{
    ...
    if (!display_disable && is->show_mode != SHOW_MODE_VIDEO && ps->audio_st) {
...
int get_video_frame(VideoState *is, AVFrame *frame)
{
    ...
    if ((got_picture = is->viddec.decode_frame(frame, NULL)) < 0)
        return -1;
    ...
    if (got_picture) {
        ...
        frame->sample_aspect_ratio = av_guess_sample_aspect_ratio(is->ic, ps->video_st, frame);
...
int configure_video_filters(AVFilterGraph *graph, VideoState *is, const char *vfilters, AVFrame *frame)
{
    ...
    PacketSource* ps = is->getPacketSource();
    AVCodecContext *codec = ps->video_st->codec;
    AVRational fr = av_guess_frame_rate(is->ic, ps->video_st, NULL);
    ...
    snprintf(buffersrc_args, sizeof(buffersrc_args),
            ...
            ps->video_st->time_base.num, ps->video_st->time_base.den,
...
#define INSERT_FILT(name, arg) do {                                          \
    ...
    if (autorotate) {
        double theta  = get_rotation(ps->video_st);
...
int configure_audio_filters(VideoState *is, const char *afilters, int force_output_format)
{
    ...
    if (is->audio_filter_src.channel_layout)
        snprintf(asrc_args + ret, sizeof(asrc_args) - ret,
                 ":channel_layout=0x",  is->audio_filter_src.channel_layout);
...
static VideoState *stream_open(const char *filename, AVInputFormat *iformat)
{
    ...
    is->read_tid     = SDL_CreateThread(PacketSource::read_thread, is);
    if (!is->read_tid) {
fail:
        stream_close(is);
        return NULL;
    ...
    for (;;) {
        ...
        case SDL_KEYDOWN:
            ...
            do_seek:
                    if (seek_by_bytes) {
                        pos = -1;
                        if (pos < 0 && cur_stream->packetSource.video_stream >= 0)
                            pos = cur_stream->pictq.last_pos();
                        if (pos < 0 && cur_stream->packetSource.audio_stream >= 0)
...
const OptionDef options[] = {
    ...
    OptionDef((const char*)"fs", OPT_BOOL, ( &is_full_screen ),(const char*)"force full screen" ),
    OptionDef((const char*)"an", OPT_BOOL, ( &audio_disable ),(const char*)"disable audio" ),
    OptionDef((const char*)"vn", OPT_BOOL, ( &video_disable ),(const char*)"disable video" ),
    OptionDef((const char*)"sn", OPT_BOOL, ( &subtitle_disable ),(const char*)"disable subtitling" ),
    OptionDef((const char*)"ast", OPT_STRING | HAS_ARG | OPT_EXPERT, ( &wanted_stream_spec[AVMEDIA_TYPE_AUDIO] ),(const char*)"select desired audio stream",(const char*)"stream_specifier" ),
    OptionDef((const char*)"vst", OPT_STRING | HAS_ARG | OPT_EXPERT, ( &wanted_stream_spec[AVMEDIA_TYPE_VIDEO] ),(const char*)"select desired video stream",(const char*)"stream_specifier" ),
    OptionDef((const char*)"sst", OPT_STRING | HAS_ARG | OPT_EXPERT, ( &wanted_stream_spec[AVMEDIA_TYPE_SUBTITLE] ),(const char*)"select desired subtitle stream",(const char*)"stream_specifier" ),
    ...
    OptionDef((const char*)"bytes", OPT_INT | HAS_ARG, ( &seek_by_bytes ),(const char*)"seek by bytes 0=off 1=on -1=auto",(const char*)"val" ),
    OptionDef((const char*)"nodisp", OPT_BOOL, ( &display_disable ),(const char*)"disable graphical display" ),
    ...
    OptionDef((const char*)"stats", OPT_BOOL | OPT_EXPERT, ( &show_status ),(const char*)"show status",(const char*)"" ),
    OptionDef((const char*)"fast", OPT_BOOL | OPT_EXPERT, ( &fast ),(const char*)"non spec compliant optimizations",(const char*)"" ),
    OptionDef((const char*)"genpts", OPT_BOOL | OPT_EXPERT, ( &genpts ),(const char*)"generate pts",(const char*)"" ),
    OptionDef((const char*)"drp", OPT_INT | HAS_ARG | OPT_EXPERT, ( &decoder_reorder_pts ),(const char*)"let decoder reorder pts 0=off 1=on -1=auto",(const char*)""),
    OptionDef((const char*)"lowres", OPT_INT | HAS_ARG | OPT_EXPERT, ( &lowres ),(const char*)"",(const char*)"" ),
    ...
    OptionDef((const char*)"autoexit", OPT_BOOL | OPT_EXPERT, ( &autoexit ),(const char*)"exit at the end",(const char*)"" ),
    OptionDef((const char*)"exitonkeydown", OPT_BOOL | OPT_EXPERT, ( &exit_on_keydown ),(const char*)"exit on key down",(const char*)"" ),
    OptionDef((const char*)"exitonmousedown", OPT_BOOL | OPT_EXPERT, ( &exit_on_mousedown ),(const char*)"exit on mouse down",(const char*)"" ),
    OptionDef((const char*)"loop", OPT_INT | HAS_ARG | OPT_EXPERT, ( &loop ),(const char*)"set number of times the playback shall be looped",(const char*)"loop count" ),
    OptionDef((const char*)"framedrop", OPT_BOOL | OPT_EXPERT, ( &framedrop ),(const char*)"drop frames when cpu is too slow",(const char*)"" ),
    OptionDef((const char*)"infbuf", OPT_BOOL | OPT_EXPERT, ( &infinite_buffer ),(const char*)"don't limit the input buffer size (useful with realtime streams)",(const char*)"" ),
    OptionDef((const char*)"window_title", OPT_STRING | HAS_ARG, ( &window_title ),(const char*)"set window title",(const char*)"window title" ),
#if CONFIG_AVFILTER
    OptionDef((const char*)"vf", OPT_EXPERT | HAS_ARG, ( (void*)(opt_add_vfilter) ),(const char*)"set video filters",(const char*)"filter_graph" ),
    OptionDef((const char*)"af", OPT_STRING | HAS_ARG, ( &afilters ),(const char*)"set audio filters",(const char*)"filter_graph" ),
#endif
    OptionDef((const char*)"rdftspeed", OPT_INT | HAS_ARG| OPT_AUDIO | OPT_EXPERT, ( &rdftspeed ),(const char*)"rdft speed",(const char*)"msecs" ),
    ...
    OptionDef((const char*)"acodec", HAS_ARG | OPT_STRING | OPT_EXPERT, (    &audio_codec_name ),(const char*)"force audio decoder",   (const char*)"decoder_name" ),
    OptionDef((const char*)"scodec", HAS_ARG | OPT_STRING | OPT_EXPERT, ( &subtitle_codec_name ),(const char*)"force subtitle decoder",(const char*)"decoder_name" ),
    OptionDef((const char*)"vcodec", HAS_ARG | OPT_STRING | OPT_EXPERT, (    &video_codec_name ),(const char*)"force video decoder",   (const char*)"decoder_name" ),
    OptionDef((const char*)"autorotate", OPT_BOOL, ( &autorotate ),(const char*)"automatically rotate video",(const char*)"" ),
...
static ssize_t stream_read (int __fd, void *__buf, size_t __nbytes, size_t off)
{
...
static int stream_fstat (int __fd, struct stat *__buf)
{
   return fstat(__fd, __buf); 
}


struct fstream_functors mystream = { stream_read, stream_fstat};
